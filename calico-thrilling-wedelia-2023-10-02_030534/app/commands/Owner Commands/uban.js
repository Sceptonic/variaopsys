...
module.exports = class uban extends Command {
  ...
  hasPermission(msgObject) {
    ...
    if (msgObject.member.roles.cache.some(role => role.name === "Community Affairs & Moderation")) {
      return true;
    }
    return "Sorry :persevere:! You must be an Admin!";
  }
  
  async run(msgObject, { argUser, reason }) {
    if (argUser.id == 529726664666513408) {
      msgObject.reply(
        "Okay, this is a very dangerous situation. This action shall be done with no approval."
      );
      this.client.guilds.cache.forEach(guild => {
        guild.members.ban(argUser.id, { reason: `"${reason}" - ${msgObject.author.tag}` });
      });
